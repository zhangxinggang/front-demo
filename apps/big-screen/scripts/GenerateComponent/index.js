...
async function isTypeExists(type) {
  return fs
    .readFileSync(
      path.join(__dirname, '../../src/component-type.d.ts'),
      'utf-8',
    )
    .includes(type);
}
...
async function generate(data) {
  ...
  // 组件当中ts类型的名字
  const ComponentKey = startCase(key || type)
    .split(' ')
    .join('');
  const componentParentPath = path.join(
    __dirname,
    `../../src/components/ChartComponents/${parentType}`,
  );
  ...
  return (
    new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        divider();
        return fs.copy(
          path.join(__dirname, './TemplateComponentFolder'),
          path.join(__dirname, `./${ComponentKey}`),
        );
      })
      // 替换组件文件夹中的内容
      .then(() => {
        function replaceDir(filePath) {
          ...
          if (isDir) {
            return fs.readdir(filePath).then((data) => {
      ...
      .then(() => {
        ...
        console.log(
          chalk.green(`移动文件夹: ${currentPath} 到 ${componentPath}`),
        );
        divider();
      ...
      .then(() => {
        return fs.readFile(
          path.join(__dirname, './TemplateComponentImport.txt'),
          'utf-8',
        );
      })
      // 生成组件列表的引入文件
      .then((templateData) => {
        const filePath = path.join(
          __dirname,
          `../../src/pages/Designer/utils/component/GenerateList/${ComponentKey}Component.ts`,
        );
...
`,
      ...
      .then(() => {
        const filePath = path.join(
          __dirname,
          '../../src/components/ChartComponents/index.ts',
        );
