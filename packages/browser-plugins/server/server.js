...
async function loadServerConfig() {
  ...
  try {
    serverConfig = await (
      await fetch(chrome.runtime.getURL('/web/resource/config.json'))
    ).json();
  } catch (error) {
    console.warn(error);
  }
  ...
  while (true) {
    ...
    try {
      if (mUrl !== uRLTrue(await getMagicUrl())) {
        //如果魔法链接变化了则重新加载
        continue;
      }
      if (!conf) {
...
async function handleRequest(request) {
  ...
  if (url.pathname === '/web/resource/cueWord.json') {
    ...
    try {
      re = await fetch(
        'https://gitee.com/jja8/NewBingGoGo/raw/master/web/resource/cueWord.json',
      );
    } catch (error) {
      console.warn(re);
    }
  ...
  try {
    ...
    if (url.pathname === '/sydney/ChatHubUrl') {
      //请求url
      async function bingChatHub(isAddRandomAddress, isSetHeaderXFFIP, url) {
        //修改请求头
      ...
      if (await getChatHubWithMagic()) {
        ...
        return await bingChatHub(
          true,
          false,
          `${mUrl.replace('http', 'ws')}/sydney/ChatHub`,
        );
      } else {
        return await bingChatHub(
          false,
    ...
    if (url.pathname === '/turing/conversation/create') {
      //创建聊天
      let mUrl = uRLTrue(await getMagicUrl());
      if (mUrl === 'https://www.bing.com') {
        //如果用的官方
      ...
      return await goUrl(
        request,
        `${mUrl}/turing/conversation/create`,
        undefined,
        true,
      );
    }

    if (url.pathname === '/edgesvc/turing/captcha/create') {
      //请求验证码图片
      let mUrl = uRLTrue(await getMagicUrl());
      if (mUrl === 'https://www.bing.com') {
        //如果用的官方
        return await goUrl(
          request,
          `https://edgeservices.bing.com/edgesvc/turing/captcha/create`,
          {
            'X-forwarded-for': request.headers.get('randomAddress'),
          },
          false,
      ...
      return await goUrl(
        request,
        `${mUrl}/edgesvc/turing/captcha/create`,
        undefined,
        true,
      );
    }

    if (url.pathname === '/edgesvc/turing/captcha/verify') {
      //提交验证码
      let mUrl = uRLTrue(await getMagicUrl());
      if (mUrl === 'https://www.bing.com') {
        //如果用的官方
        return await goUrl(
          request,
          `https://edgeservices.bing.com/edgesvc/turing/captcha/verify?` +
            url.search,
          {
            'X-forwarded-for': request.headers.get('randomAddress'),
          },
      ...
      return await goUrl(
        request,
        `${mUrl}/edgesvc/turing/captcha/verify?` + url.search,
        undefined,
        true,
      );
    }

    if (url.pathname === '/msrewards/api/v1/enroll') {
      //加入候补
      let mUrl = uRLTrue(await getMagicUrl());
      let addHeaders = {};
      let withR = true;
      ...
      return await goUrl(
        request,
        `${mUrl}/msrewards/api/v1/enroll` + url.search,
        addHeaders,
        withR,
      );
    }
    ...
    if (url.pathname.startsWith('/images/create/async/results')) {
      ...
      let a =
        url.pathname.replace(
          ...
          `${mUrl}/images/create/async/results`,
        ) + url.search;
...
async function getChatHubWithMagic() {
  return !!(await chrome.storage.local.get('ChatHubWithMagic'))
    .ChatHubWithMagic;
}
...
function uRLTrue(magicUrl) {
  ...
  //如果带协议头
  if (
    new RegExp('^(https?://)([-a-zA-z0-9]+\\.)+([-a-zA-z0-9]+)+\\S*$').test(
      magicUrl,
    )
  ) {
    return mu(magicUrl);
...
async function goUrl(request, url, addHeaders, withRandomAddress) {
  ...
  if (!res.ok) {
    if (res.headers.has('cf-mitigated')) {
      ...
      let error = new Error(
        `<p>被质疑为恶意攻击,需要通过机器人校验。</p><p><a href="${challengeUrl}" target="_blank">点击前往校验。</a></p>`,
      );
      ...
      addRules: [
        {
          ...
          condition: {
            regexFilter:
              '^https://hm.baidu.com/hm.gif?(.*)si=b435e427dc3b96eba3fc5df18958e020(.*)',
            resourceTypes: ['image'],
          },
        },
      ],
