...
async function start() {
  ...
  try {
    res = await nBGGFetch(
      `${window.location.origin}/edgesvc/turing/captcha/create`,
      {
  ...
  theSub.onclick = async () => {
    ...
    try {
      res = await nBGGFetch(
        `${window.location.origin}/edgesvc/turing/captcha/verify?${q.toString()}`,
        {
          headers: { cookieID: cookieId },
        },
      );
    ...
    if (json.statusCode === 200 && json.reason === 'Solved') {
      // ||json.reason==="NoChallengeSession"
      theH1.innerText = `已解决！✅`;
      theImg.remove();
      theInput.remove();
      theSub.remove();
