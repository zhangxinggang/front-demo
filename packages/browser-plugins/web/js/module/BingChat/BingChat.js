...
export default class BingChat {
  ...
  async start(theChatType) {
    ...
    try {
      res = await nBGGFetch(
        `${window.location.origin}/turing/conversation/create`,
        !CookieID.cookieID
          ? undefined
          : { headers: { cookieID: CookieID.cookieID } },
      );
    } catch (e) {
      console.warn(e);
      throw e.isNewBingGoGoError
        ? e
        : new Error('无法连接到web服务器，请刷新页面重试:' + e.message);
    }
    let cookieID = res.headers.get('cookieID');
    if (res.status === 404) {
      throw new Error(
        `服务所在地区不提供NewBing服务，请联系服务搭建者切换服务所在地区，第${cookieID}个账号。`,
      );
    }
    ...
    if (rText.length < 1) {
      if (cookieID === 'self') {
        throw new Error(
          `魔法链接服务服务所在地区无法使用或账号登录状态失效，尝试切换魔法链接或重新登录。`,
        );
      } else {
        throw new Error(
          `服务所在地区无法使用或cookie失效，第${cookieID}个账号。`,
        );
      }
    }
    ...
    if (resjson.result.value !== 'Success') {
      ...
      if (resjson.result.value === 'UnauthorizedRequest') {
        ...
        mess =
          cookieID === 'self'
            ? '在使用之前需要先登录微软账号，请前往bing.com登录微软账号。'
            : `cookie失效，第${cookieID}个cookie。`;
      } else if (resjson.result.value === 'Forbidden') {
        type = 'NoPower';
        mess =
          cookieID === 'self'
            ? '当前账号没有使用NewBing的权限'
            : `cookie无权限，第${cookieID}个cookie。`;
      }
      let error = new Error(mess);
      error.type = type;
      throw error;
    }
    this.bingChating = BingChating.create(
      this,
      resjson.conversationId,
      resjson.clientId,
      resjson.conversationSignature,
      theChatType,
      undefined,
    );
