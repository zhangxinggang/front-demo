...
export default class ParserReturnWorker {
  ...
  constructor(chatDiv, chatSuggestionsWorker) {
    ...
    window.addEventListener('message', (event) => {
      let data = event.data;
      if (
        data.type === 'AnswerCardDimensionUpdate' ||
    ...
    chatDiv.addEventListener('click', async function (event) {
      ...
      if (event.target.classList.contains('copy-bingcat-markdown')) {
        ...
        await navigator.clipboard.writeText(
          messageElement.dataset.the_markdown_text,
        );
      }
  ...
  addNoPower() {
    ...
    go.onclick = async () => {
      ...
      try {
        await nBGGFetch(
          `${window.location.origin}/msrewards/api/v1/enroll?publ=BINGIP&crea=MY00IA&pn=bingcopilotwaitlist&partnerId=BingRewards&pred=true&wtc=MktPage_MY0291`,
        );
        go.innerHTML =
          '请求成功！请刷新页面重试，如果无权限使用请等待几天后重试。';
      } catch (error) {
  ...
  addNoLogin() {
    ...
    this.chatDiv.appendChild(go);
    go.href =
      'https://login.live.com/login.srf?wa=wsignin1.0&rpsnv=13&id=264960&wreply=https%3A%2F%2Fcn.bing.com%2Fsecure%2FPassport.aspx%3Fedge_suppress_profile_switch%3D1%26requrl%3Dhttps%253a%252f%252fcn.bing.com%252f%253fwlexpsignin%253d1&wp=MBI_SSL&lc=2052&aadredir=1';
    go.target = '_blank';
  }
    ...
    if (item.result) {
      ...
      if (result.value === 'Success') {
        ...
        if (window.location.protocol === 'chrome-extension:') {
          this.addError(
            '当前账号请求过多，需要通过机器人检查！无法通过请等待24小时后再试。',
          );
          ...
          this.addError(
            `当前账号请求过多，需要通过机器人检查！第${CookieID.cookieID}个账号`,
          );
        }
        ...
        this.addError(
          `<p><a href="./ChatImgCAPTCHA.html?${p.toString()}">点击前往验证</a></p>`,
        );
      } else {
        this.addError(result.message);
   ...
   * */
  addCAPTCHA() {
    let div = this.getByID(
      new Date().getTime() + 'CAPTCHA',
      'div',
      this.chatDiv,
    );
  ...
  porserMessages(messages, father) {
    for (let i = 0; i < messages.length; i++) {
      ...
      if (!message.messageType && message.adaptiveCards) {
        //如果是正常的聊天消息
        let adaptiveCardsFatherDIV = this.getByID(
          message.messageId,
          'div',
          father,
        ...
        if (message.sourceAttributions) {
          if (message.sourceAttributions.length > 0) {
            let sourceAttributionsDIV = this.getByID(
              message.messageId + 'sourceAttributions',
              'div',
              father,
              'sourceAttributions',
            );
            this.porserSourceAttributions(
              message.sourceAttributions,
              sourceAttributionsDIV,
            );
          }
        }
        ...
        this.porserLineTextBlocks(message.text, div);
      } else if (message.messageType === 'GenerateContentQuery') {
        //如果是生成内容查询
        let div = this.getByID(
          message.messageId,
          'div',
      ...
      } else if (message.messageType === 'RenderCardRequest') {
        //渲染卡片请求，目前不知道有什么用
  ...
  renderCardRequest(message, father) {
    ...
    if (father[messageId + 'renderCardRequest']) {
      //防止解析多次
      return;
    }
  ...
  porserAdaptiveCards(adaptiveCards, father) {
    for (let i = 0; i < adaptiveCards.length; i++) {
      ...
      if (adaptiveCard.type === 'AdaptiveCard') {
        ...
        console.log(
          '发现一个不是AdaptiveCard的adaptiveCard',
  ...
  porserTextBlock(body, father) {
    if (!body.size) {
      ...
      if (
        div.dataset.the_markdown_text &&
        div.dataset.the_markdown_text.length > body.text.length
      ) {
        ...
        endDiv.innerHTML =
          '[以上被撤回的信息可能包不适宜的内容，已被隐藏。] <input type="checkbox">显示';
        insertAfter(endDiv, div);
        let newDiv = this.getByClass(
          'textBlock',
          'div',
          father,
          'markdown-body',
...
<div>${this.throttling.numUserMessagesInConversation} / ${this.throttling.maxNumUserMessagesInConversation}</div>`;
  ...
  porserSourceAttributions(sourceAttributions, father) {
    ...
    for (let i = 0; i < sourceAttributions.length; i++) {
      ...
      html =
        html +
        `<a target="_blank" href="${sourceAttribution.seeMoreUrl}">${sourceAttribution.providerDisplayName}</a>`;
    }
