...
export default class ChatModeSwitchingWorker {
   ...
   * */
  updateShow() {
    if (this.chatType === ChatModeSwitchingWorker.ChatType.Creative) {
      //有创造力的
    ...
    } else if (this.chatType === ChatModeSwitchingWorker.ChatType.Balanced) {
      //平衡
    ...
    } else if (this.chatType === ChatModeSwitchingWorker.ChatType.Precise) {
      //精确的
      this.chatTypeChoseCreate.classList.remove('Chose');
      this.chatTypeChoseBalance.classList.remove('Chose');
  ...
  onChatTypeChange(chatType, isUser) {
    console.debug(
      `onChatTypeChange方法没有被重写！,聊天类型切换到'${chatType}'`,
    );
  }
