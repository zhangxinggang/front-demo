...
window.addEventListener('load', async () => {
  ...
  const bingChat = new BingChat(
    new ChatFirstMessages_Small(),
    new ChatOptionsSets_Small(),
  ); //聊天对象 BingChat 对象
  //加载需要用到的对象
  ...
  async function reSetStartChatMessage() {
    ...
    chatSuggestionsManager.set(
      await bingChat.chatFirstMessages.nextStartProposes(),
    );
   ...
   * */
  function onPreview() {
    previewMessageID = parserReturnMessage.setMyChat(
      getSendMessage(),
      previewMessageID,
      true,
    );
