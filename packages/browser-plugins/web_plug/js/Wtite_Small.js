...
window.addEventListener('load', async () => {
  ...
  const titleManager = new TitleWorker(document.getElementById('goGoSubtitle'));
  ...
  selectButtonFunRetrun(toneSelectbuttns, (re) => {
    ...
    findButton(toneSelectbuttns, url.searchParams.get('tone')),
  );

  selectButtonFunRetrun(formatSelecctbuttns, (re) => {
    chatOptionsSets_Write.format = re.dataset.value;
  })(formatSelecctbuttns[0])(
    findButton(formatSelecctbuttns, url.searchParams.get('format')),
  );

  selectButtonFunRetrun(lengthSelectbuttns, (re) => {
    chatOptionsSets_Write.length = re.dataset.value;
  })(lengthSelectbuttns[0])(
    findButton(lengthSelectbuttns, url.searchParams.get('length')),
  );
  ...
  async function reSetStartChatMessage() {
    parserReturnMessage.restart(
      await bingChat.chatFirstMessages.nextStartMessage(),
    );
    titleManager.restart();
  }
  ...
  async function send(text) {
    ...
    try {
      isSpeakingStart();
      returnMessage = await bingChat.sendMessage(
        text,
        parserReturnMessage.getOnMessageFun((even) => {
          if (
            even.type === 'close' ||
            even.type === 'close-accident' ||
            even.type === 'error'
          ) {
            isSpeakingFinish();
          }
  ...
  //如果有发送第一条消息的参数
  let sendMessage = new URL(window.location.href).searchParams.get(
    'sendMessage',
  );
  if (sendMessage) {
